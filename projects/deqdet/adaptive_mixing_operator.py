...
class AdaptiveMixing(nn.Module):
    def __init__(self, in_dim, in_points, n_groups, query_dim=None,
        ...
        # #TODO: add cache
        # self.cache_weight = nn.Linear(self.query_dim, self.n_groups)
        # self.cache_norm = nn.LayerNorm([self.n_groups, self.out_points, self.eff_out_dim])
        # def ema_update(query, cache, current):
        #     # import pdb
        #     # pdb.set_trace()
        #     # weight = self.cache_weight(query).view(-1, self.n_groups, 1, 1)#.sigmoid()
        #     out = cache + current #cache*weight + current*(1-weight)
        #     out = self.cache_norm(out)
        #     return out
    ...
    def forward(self, x, query):
        ...
        # out = self.ema_update(query, self.cache, out) if getattr(self, 'cache', None) is not None else out
